...
.
Spring Boot 3.x/4: Framework base para a construção da API
.
...
O projeto adota a estrutura de Monólito Modular, inspirada em Clean Architecture, para garantir baixo acoplamento e alta coesão
. O código é organizado nos seguintes módulos/pacotes:
...
Finalidade
Hibernate Envers
Diff de campos (antes x depois)
Histórico imutável de alterações nos dados
.
Spring AOP
Contexto HTTP (IP, Usuário, Endpoint)
Identificação da origem e autoria da requisição
.
🛠️ Como Executar
Pré-requisitos
Java 21 JDK
...
Configurar variáveis de ambiente: Copie o arquivo .env.example para .env e ajuste as credenciais se necessário
.
Subir a infraestrutura (PostgreSQL):
...
.
GET
/api/pix/transferencias/{id}
Busca detalhes de uma transferência
.
GET
/api/pix/transferencias/{id}/historico
Retorna o histórico de revisões (Envers)
.
PATCH
/api/pix/transferencias/{id}/cancelar
Realiza o cancelamento de uma transferência
.
GET
/api/pix/audit
Consulta logs de contexto (AOP) com filtros
.